...
public class CarsView extends VerticalLayout {
    ...
    @Autowired
    public CarsView(WebClient webClient) {
    ...
    private Component createStatsCards() {
        ...
        int totalCars = fetchCars().size();
        long validatedCars = fetchCars().stream()
    ...
    private void openCarDialog(CarDto car) {
        ...
        saveButton.addClickListener(e -> {
            try {
                ...
                if (isUpdate) {
                    CarDto req = new CarDto();
                    req.setMake(car.getMake());
                    req.setModel(car.getModel());
                    req.setLicensePlate(car.getLicensePlate());
                    req.setCarValidated(car.isCarValidated());
                    updateCar(car.getId(), req);
    ...
    private void refreshGrid() {
        grid.setItems(fetchCars());
    }

    private java.util.List<CarDto> fetchCars() {
    ...
    private CarDto createCar(CarDto request) {
        return webClient.post()
            .uri("/api/cars")
            .contentType(MediaType.APPLICATION_JSON)
            .body(Mono.just(request), CarDto.class)
            .retrieve()
            .bodyToMono(CarDto.class)
            .block();
    }

    private CarDto updateCar(java.util.UUID id, CarDto request) {
    ...
    private void deleteCarById(java.util.UUID id) {
        webClient.delete()
            .uri("/api/cars/{id}", id)
            .retrieve()
            .toBodilessEntity()
